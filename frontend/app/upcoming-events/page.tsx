...
const mapEventToCard = (event: any) => ({
  ...
  typeEn: event.type,
  typeAr: event.type,
  cityEn: [event.custom_venue_name, event.city_name].filter(Boolean).join(", ") || event.venue_name_en || event.venue_city_en || "",
  cityAr: [event.custom_venue_name, event.city_name].filter(Boolean).join(", ") || event.venue_name_ar || event.venue_city_ar || "",
  ...
  seats: event.max_attendees ? Number(event.max_attendees).toLocaleString() : "TBA",

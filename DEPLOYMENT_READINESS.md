...
## Laravel API: api.stylishmice.com

1. Upload the Laravel backend to the API hosting account.
2. Set the `api.stylishmice.com` document root to `backend/public`.
...
5. Configure production `.env` without printing secrets:
   - `APP_NAME`, `APP_ENV=production`, `APP_KEY`, `APP_DEBUG=false`, `APP_URL=https://api.stylishmice.com`
   ...
   - `FRONTEND_URL=https://stylishmice.com`, `FRONTEND_URLS=https://stylishmice.com,https://www.stylishmice.com`
...
## Next.js Frontend: stylishmice.com

1. Configure production environment:
   - `NEXT_PUBLIC_API_BASE_URL=https://api.stylishmice.com`
   - `NEXT_PUBLIC_BOOKING_API_URL=https://api.stylishmice.com`
   - `NEXT_PUBLIC_SITE_URL=https://stylishmice.com`
...
1. API: `GET https://api.stylishmice.com/health`
2. API public: `GET https://api.stylishmice.com/api/public/events`
3. API protected: `GET https://api.stylishmice.com/api/me/dashboard` should return JSON `401` without a token.
4. Frontend: open `https://stylishmice.com`, `/upcoming-events`, `/previous-events`, `/about`, `/contact`, `/login`, `/signup`, `/admin`, and `/dashboard`.
